'''
Ứng dụng trê trong những bài toán đơn giản.
Bài toán: Viết chương trình theo các yêu cầu sau:
1.Nhập dữ liệu cho cây nếu mã sinh viên trùng thì thông báo trùng và nhập
2.Duyệt và xuất dữ liệu của cây theo thứ tự LNR ra màn hình
3. Sắp xếp cây theo trường dữ liệu
4. Đếm số nút lá của cây
5. Tính chiều cao của cây
6. Chèn 1 Node vào cây
7. Tìm kiếm một Node có giá trị nhập vào từ bàn phím
8. Xóa 1 Node
'''
#Code
import os
class Node:
    ...
    def tim_kiem(self, masv, goc = 0):
        #Tìm kiếm một Node có giá trị nhập vào từ bàn phím
        nut_ht = goc
        if goc == 0:
            nut_ht = self
        if nut_ht is None:
...
def main():
    while True:
        ...
        print('2. Duyệt và xuất dữ liệu của cây theo thứ tự LNR')
        print('3. Sắp xếp cây theo trường dữ liệu')
        print('4. Đếm số nút lá của cây')
        print('5. Tính chiều cao của cây')
        print('6. Chèn 1 Node vào cây')
        print('7. Tìm kiếm một Node có giá trị nhập vào từ bàn phím')
        print('8. Xóa 1 Node')
        print('9. Thoát')
